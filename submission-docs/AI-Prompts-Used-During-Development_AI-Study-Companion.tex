...
\title{AI Study Companion\\AI Prompts Used During Development}
...
Agent / models: OpenCode coding agent $\cdot$ Claude (planning) $\cdot$ Muse Spark (implementation sessions)\\
Raw history: \texttt{extra-docs/docs/development-prompts/} (19 per-phase logs: prompt + purpose + result + deviations)\\
Status: 314 tests passing $\cdot$ 18 eval fixtures green
...
I worked in strict build order --- one prompt at a time, acceptance checks green before moving on,
\texttt{/compact} between phases. The prompts below are the \textit{material} ones (lightly cleaned
for readability: typos fixed, secrets redacted), grouped the way the submission asks --- architecture,
frontend, backend, database, AI, debugging, testing, documentation and deployment.
Each shows \textbf{context $\rightarrow$ prompt $\rightarrow$ why it mattered $\rightarrow$ outcome}.
The per-phase folder in the repo holds the verbatim logs.
\end{shaded}
...
\subsection{P1 --- ``Help me think before I code'' (planning with Claude)}
\paragraph{Context:} Day 0, before any code.
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Why it mattered:} this produced \texttt{architecture.md} --- single source of truth --- and the MUST/SHOULD/FUTURE scope that protected the timeline.\\
\textbf{Outcome:} 19 phases in dependency order; ``no microservices / no second vector DB / no custom auth'' written down so I never reconsidered mid-build.

\subsection{P2 --- ``Turn the architecture into agent-ready tasks''}
\paragraph{Context:} Still planning, about to start OpenCode.
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Why it mattered:} produced \texttt{build-prompts.md} --- I never had to design-while-coding.\\
\textbf{Outcome:} phases 02--19 executable in order; every phase log follows the same template (prompt / purpose / result / changes / notes / compact).

\subsection{P3 --- ``Scaffold it, nothing fancy yet''}
\paragraph{Context:} To OpenCode, Phase 02, fresh repo.
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Outcome:} clean booting skeleton; every later phase had somewhere to land.\\
\textbf{Lesson:} keeping P3 logic-free avoided half-wired imports haunting me for days.

% =====================================================================
\section{Database}
% =====================================================================

\subsection{P4 --- ``Write the schema like you mean the RLS''}
\paragraph{Context:} To OpenCode, Phase 03.
\begin{lstlisting}
Using Supabase Postgres + pgvector, can you write the migrations under db/schema/ for the full schema in architecture.md section 6 (spaces -> ... -> ai_operations)? uuid PKs, timestamptz, CASCADEs, ivfflat/hnsw on chunks.embedding, indexes on every project_id/user_id + learning_events(project_id, created_at), and RLS on everything user-owned (`user_id = auth.uid()`). I started chunks at 1536 in the draft -- flag whatever dim you actually use and keep it consistent. Add a README for running it. Acceptance: migrates clean on a fresh project and RLS blocks cross-user reads. Then /compact.
\end{lstlisting}
\textbf{Outcome:} migrations 001 (+ 002 retrieve, 003 storage); later 004--014 layered embeddings 384 -> Gemini 768, summaries, practice, dedupe, pinning.\\
\textbf{Deviation I chose:} embedding dim moved twice (1536 -> 384 -> 768) --- each with its own migration + guards rather than editing history.

\subsection{P5 --- ``Auth that actually redirects properly''}
\paragraph{Context:} To OpenCode, Phase 04.
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Outcome:} dual helpers + \texttt{isAuthError} $\rightarrow$ 401 mapping; route-audit tests lock it in. This one decision killed a whole class of flaky ``sometimes 500'' bugs.

% =====================================================================
\section{Backend --- spaces, projects, materials \& jobs}
% =====================================================================

\subsection{P6 --- ``CRUD where you can't read anyone else's stuff''}
\paragraph{Context:} To OpenCode, Phase 05.
\begin{lstlisting}
Spaces + Projects CRUD please, fully ownership-scoped. Every service lookup must be `WHERE id AND user_id` -- I don't want to rely on remembering to call a separate check. Thin route handlers only. /dashboard lists spaces, /spaces/[id] lists projects (404 -- not 403 -- for other people's stuff), project page can be a placeholder for now. Emit SPACE_CREATED + PROJECT_CREATED. I'll verify with two accounts that A's URLs 404 for B. Then /compact.
\end{lstlisting}
\textbf{Outcome:} \texttt{project.service.ts} pattern reused by every later service; ownership tests still assert it.

\subsection{P7 --- ``Upload returns fast, processing happens behind the scenes''}
\paragraph{Context:} To OpenCode, Phase 06.
\begin{lstlisting}
Material upload + background pipeline next. Sync part: validate PDF (size cap), store in the private bucket under user/project, create materials row QUEUED, emit event, fire Inngest, return 202 -- don't block. Async (Inngest): PROCESSING -> extract text with page numbers -> chunk ~500-800 tokens w/ overlap -> embed via AIService -> store chunks -> extract concepts (structured, max 8) -> READY; any failure -> FAILED + processing_error, never stuck. Materials page needs QUEUED/PROCESSING/READY/FAILED + Retry. I'll upload a real PDF and a corrupt one. Then /compact.
\end{lstlisting}
\textbf{Outcome:} pipeline + \texttt{AIService} skeleton + status UI. Later hardened with SHA-256 dedupe, atomic claims, OCR for scans, Gemini batching, and the inline fallback (see P19).

\subsection{P8 --- ``Home dashboard that answers: where was I, how am I doing, what's next?''}
\paragraph{Context:} To OpenCode, continuation task, PRD section 16 gap.
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Outcome:} Home now matches the PRD's ``where was I / how am I doing / what next'' bar; per-source try/catch keeps it resilient.

\subsection{P9 --- ``Admin health I can actually look at during the demo''}
\paragraph{Context:} To OpenCode, continuation, before submission.
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Outcome:} health view + \texttt{admin-health.test.ts}; doubles as my pre-demo checklist.

% =====================================================================
\section{AI --- retrieval, Tutor, quiz \& grading}
% =====================================================================

\subsection{P10 --- ``Retrieval that can say `I don't know'\,''}
\paragraph{Context:} To OpenCode, Phase 07.
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Outcome:} \texttt{lib/rag/retrieve.ts} (threshold 0.25, \texttt{match\_chunks} + JS fallback); the insufficient path is the backbone of the whole ``evidence over guessing'' story.

\subsection{P11 --- ``The Tutor: grounded, cited, and stubborn about evidence''}
\paragraph{Context:} To OpenCode, Phase 08, the core grading surface.
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Outcome:} grounded Tutor + injection defense + observability from day one; fixtures TUTOR-01 to TUTOR-05 still guard it.

\subsection{P12 --- ``Quiz picking that's smarter than wrong-to-easy''}
\paragraph{Context:} To OpenCode, Phase 09.
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Outcome:} weighted selector + difficulty/type picker; the ``not naive'' requirement is literally asserted in tests.

\subsection{P13 --- ``Grade MCQ with code, open-ended with care''}
\paragraph{Context:} To OpenCode, Phase 10.
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Outcome:} deterministic MCQ, explanatory open-ended grading ($\geq$60 means correct), per-question idempotency including race handling, answer-gating (answers hidden until graded).

\subsection{P14 --- ``Mastery is math, not vibes --- plus growth + recommendations that name names''}
\paragraph{Context:} To OpenCode, Phases 11--12 (I combined the review).
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Outcome:} explainable mastery, growth bands, specific recommendations (skipped when nothing weak --- by design); \texttt{mastery\_history} is what makes growth + future spaced-repetition possible.

% =====================================================================
\section{Frontend --- project hub, dashboards \& admin}
% =====================================================================

\subsection{P15 --- ``Project page that fans out without falling over''}
\paragraph{Context:} To OpenCode, mid-build UI pass.
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Outcome:} resilient hub; the pattern was copied to dashboard + admin pages.

\subsection{P16 --- ``Analytics with numbers I can recount by hand''}
\paragraph{Context:} To OpenCode, Phase 13.
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Outcome:} \texttt{analytics.service.ts} (project + global) with ownership-checked queries; admin engagement/learning views reuse the same aggregations.

\subsection{P17 --- ``Admin: allow-list for now, but the full drill-down''}
\paragraph{Context:} To OpenCode, Phase 14.
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Outcome:} 11 admin views including engagement, learning, health, and run-over-run eval comparison --- the ``observable AI'' story reviewers can click through.

\subsection{P18 --- ``Make the sidebar feel like one product''}
\paragraph{Context:} To OpenCode, late UI polish.
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Outcome:} unified shell; small change, big ``real product'' feel in the demo video.

% =====================================================================
\section{Debugging --- the ones that saved the submission}
% =====================================================================

\subsection{P19 --- ``Materials stuck in QUEUED on Vercel --- help me make this bulletproof''}
\paragraph{Context:} To OpenCode, pre-deploy fire-drill.
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Outcome:} the strongest pipeline in the repo; demo-day insurance. Logged in \texttt{17-debugging.md}.

\subsection{P20 --- ``Switch embeddings to Gemini 768 without breaking retrieval''}
\paragraph{Context:} To OpenCode, provider migration.
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Outcome:} single-provider Gemini space; \texttt{match\_chunks(vector(768))} + guards enforce it; stale FastEmbed/Groq docs were the main thing I had to correct afterwards (see P25).

\subsection{P21 --- ``Build is failing on a circular type in eval fixtures --- smallest fix?''}
\paragraph{Context:} To OpenCode, broken build, late at night.
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Outcome:} one-type fix, build green; captured in the phase-15 log as a reminder that eval fixtures are load-bearing.

\subsection{P22 --- ``Tutor answers are leaking between projects?? (false alarm --- prove it)''}
\paragraph{Context:} To OpenCode, security scare, turned out fine.
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Outcome:} verified \texttt{project\_id + user\_id} scoping throughout + RLS; residual note (service-role background reads) honestly recorded in limitations rather than hidden.

% =====================================================================
\section{Testing \& evaluation}
% =====================================================================

\subsection{P23 --- ``Tests that protect the grade, not coverage theatre''}
\paragraph{Context:} To OpenCode, Phase 16A.
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Outcome:} 314 tests / 26 files, all green --- the safety net that let me refactor (Gemini move, sidebar rework) without fear.

\subsection{P24 --- ``An eval suite I can re-run after every prompt tweak''}
\paragraph{Context:} To OpenCode, Phase 16B + continuation.
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Outcome:} regression-aware eval; prompt/model/retrieval edits are now diffed, not silently shipped.

% =====================================================================
\section{Documentation \& deployment}
% =====================================================================

\subsection{P25 --- ``Docs pass: make everything true again''}
\paragraph{Context:} To OpenCode, Phase 19, pre-submission.
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Outcome:} this submission set; the audit caught exactly the drift (FastEmbed-384/Groq mentions, 158-vs-314 counts) a reviewer would have flagged.

\subsection{P26 --- ``Deploy it and smoke-test the whole loop live''}
\paragraph{Context:} To OpenCode, Phase 18 (I ran the live pass by hand).
\begin{lstlisting}
...
\end{lstlisting}
\textbf{Outcome:} live at \url{https://ai-study-companion-three-inky.vercel.app/}; hardening (inline fallback, maxDuration, Gemini purge+reindex) came out of that manual pass --- the kind of thing AI can't do for you.

% =====================================================================
\section{What I learned about prompting (and what I'd do differently)}
% =====================================================================

\begin{longtable}{p{4.5cm}p{10.5cm}}
\toprule
\textbf{Lesson} & \textbf{What I'll repeat} \\
\midrule
\endfirsthead
\toprule
\textbf{Lesson} & \textbf{What I'll repeat} \\
\midrule
\endhead
Plan once, in writing, before coding &
architecture.md + phased prompts meant I never paid ``re-design tax'' mid-sprint. The /compact rhythm kept context fresh across 19 phases. \\
\midrule
Put acceptance checks in the prompt, not after &
``I'll verify X by doing Y'' forced testable output --- and gave me the eval fixtures almost for free. \\
State the non-goal explicitly &
``No backend logic yet'', ``don't refactor the whole service'', ``fail loudly on dim mismatch'' saved more time than any clever wording. \\
Keep the human as reviewer, on record &
Every phase log has Changes/Notes --- where I overruled the agent (recommendation specificity, 0.7/0.3 simplicity, allow-list over fake RBAC). That is what makes this ``my original work'' honestly. \\
Debug prompts need symptoms + constraints &
P19--P22 worked because I pasted the failure, named what must not break, and asked for the minimal fix --- not ``make it better''. \\
Docs rot faster than code &
P25's ``hunt stale claims against the code'' step should have run after every provider change, not just at the end. Next time: docs check inside each phase's acceptance criteria. \\
\bottomrule
\end{longtable}

\begin{shaded}
\noindent\textit{Originality statement: this deliverable is my original work. AI tools (above) assisted
with planning, implementation, debugging and drafting under my direction; the architecture decisions,
acceptance testing, live deployment, smoke tests, demo video, and these submission documents were
produced and verified by me. Provider keys, accounts and the deployed instance are mine; no
third-party code is presented as my own.}
\end{shaded}
