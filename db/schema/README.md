...
- The `chunks.embedding` column is `VECTOR(768)` (Google Gemini `gemini-embedding-2` via `@google/genai`, `outputDimensionality=768`, instruction-prefixed: documents `title: ... | text: ...`, queries `task: search result | query: ...`) with an IVFFlat index for cosine similarity search — current and only embedding model (document and query vectors always share it). `012_embeddings_gemini2_768.sql` historically purged incompatible `gemini-embedding-001` chunks (same 768 dims, different embedding space; earlier bge-small 384d / nomic 768d were purged by `006`). Never mix dimensions/models.
- Clean-reset note (2026-09-18): old testing/failed learning and indexing rows were deleted transactionally (FK-safe order, storage objects removed via the Storage API); no embeddings were migrated or re-embedded. Schema, RLS, indexes, `match_chunks(vector(768))`, and `auth.users` were preserved — the DB starts from a clean dataset for fresh `gemini-embedding-2` materials.
