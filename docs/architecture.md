...
| LLM provider | Mercury (Inception Labs) for chat/structured/evaluation + Google Gemini for embeddings (via thin `AIService` abstraction) | Mercury `mercury-2.5`, OpenAI-compatible + JSON mode (`response_format: json_object`) + server-side validation, with reasoning-model shaping (token headroom, temp clamp, `reasoning_effort=low` for structured tasks); embeddings: Gemini `gemini-embedding-001` (768 dims, `VECTOR(768)`, `GEMINI_API_KEY` server-only) because neither Mercury nor Meta exposes an embeddings endpoint (both verified 404). The retired local FastEmbed Docker service (`embeddings/`, 384 dims) was removed. See `lib/ai/AIService.ts` header. |
