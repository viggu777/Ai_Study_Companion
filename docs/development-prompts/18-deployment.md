...
- **Env parity:** `.env.example` matches architecture §17 exactly: `DATABASE_URL, NEXT_PUBLIC_SUPABASE_URL, NEXT_PUBLIC_SUPABASE_ANON_KEY, SUPABASE_SERVICE_ROLE_KEY, MERCURY_API_KEY, MERCURY_API_BASE_URL, MERCURY_CHAT_MODEL, META_API_KEY, META_API_BASE_URL=https://api.llama.com/compat/v1, EMBEDDING_PROVIDER, EMBEDDING_API_BASE_URL, EMBEDDING_MODEL, GROQ_API_KEY, INNGEST_EVENT_KEY, INNGEST_SIGNING_KEY, INNGEST_DEV` + prototype `ADMIN_EMAILS/ADMIN_USER_IDS`. `.gitignore` covers `.env, .env.local, .env.*.local` (+ `node_modules, .next`) and generated eval outputs (`evaluation-results.json`, `tests/eval/results.json`, `tests/eval/history/`). Secret scan: no `eyJhbGci` service key or private URL committed outside `node_modules/.next` (only public `https://chorjipooxjrmvswnrdq.supabase.co` in docs, which is `NEXT_PUBLIC_` by design).
...
- **Manual steps remaining (require Vercel/Supabase dashboard access, not doable from here):**
  ...
  2. Vercel → Settings → Environment Variables: paste all keys from `.env.example` (use production Supabase URL/keys, real `MERCURY_API_KEY` (or `META_API_KEY` fallback) + embeddings/Groq/Inngest keys from their dashboards, matching architecture §17). Redeploy.
...
- As-built update (2026-09-17, Task 5): provider split has since changed — chat
  defaults to Mercury (`MERCURY_API_KEY`, model `mercury-2.5`) with Meta as fallback,
  embeddings default to local `BAAI/bge-small-en-v1.5` (384 dims, `004` migration;
  Groq 768 only as opt-in fallback). The repo is now git-initialized, but deployment
  itself is still not done (no Vercel URL, no Inngest Cloud sync). Current providers:
  see `lib/ai/AIService.ts` header and `docs/architecture.md` §17.
